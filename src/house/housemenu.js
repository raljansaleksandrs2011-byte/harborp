// HOUSE //
global.house = mp.browsers.new('package://browser/modules/HouseInfo/index.html');
// global.jobs = mp.browsers.new('package://browser/jobs.html');

mp.events.add('HouseMenu', (id, Owner, Type, Locked, Price, Garage, Roommates, housetype) => {
	if (global.menuCheck()) return;
    menuOpen();
	house.execute(`hm.set('${id}','${Owner}','${Type}','${Locked}','${Price}','${Garage}','${Roommates}','${housetype}')`);
...
mp.events.add('HouseMenu1', (id, Owner, Type, Locked, Price, Garage, Roommates, housetype) => {
	if (global.menuCheck()) return;
    menuOpen();
	house.execute(`hm2.set('${id}','${Owner}','${Type}','${Locked}','${Price}','${Garage}','${Roommates}','${housetype}')`);
...
mp.events.add('HouseMenuBuy', (id, Owner, Type, Locked, Price, Garage, Roommates, housetype) => {
	if (global.menuCheck()) return;
    menuOpen();
	house.execute(`hmBuy.set('${id}','${Owner}','${Type}','${Locked}','${Price}','${Garage}','${Roommates}','${housetype}')`);
