const positions = [
	...
	{ 'position': {"x":1372.8314,"y":-2077.466,"z":50.87853}      , 'color': 4 },
]
...
mp.events.add('ARREA:DRUGS:DELIVERY', function (state, pos) {
	if (state) {
	   const blip = mp.game.ui.addBlipForRadius(pos.x, pos.y, pos.z, 300);
...
let Sprinter;
mp.events.add('ARREA:BLIPS:MSPRINTER', function (state, pos) {
	if (state) {
	   const blip = mp.game.ui.addBlipForRadius(pos.x, pos.y, pos.z, 40);
	   mp.game.invoke(getNative("SET_BLIP_SPRITE"), blip, 4);
	   mp.game.invoke(getNative("SET_BLIP_ALPHA"), blip, 255);
	   mp.game.invoke(getNative("SET_BLIP_COLOUR"), blip, 47);
	   Sprinter = blip;
		...
		mp.game.invoke(getNative("SET_BLIP_ALPHA"), Sprinter, 0);
});