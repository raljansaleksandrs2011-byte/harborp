...
mp.events.add('UpdateGreenZone', state => {
    mp.gui.execute(`HUD.greenzone=${state}`);
});

mp.events.add('UpdateFishZone', state => {
    mp.gui.execute(`HUD.fishzone=${state}`);
});
...
mp.events.add('AddCashMoney', function(state, cashmoney) {
    mp.gui.execute(`HUD.cashm=${state}`);
    mp.gui.execute(`HUD.cashmoney=${cashmoney}`);
	// mp.game.audio.playSoundFrontend(-1, 'Checkpoint_Cash_Hit', 'GTAO_FM_Events_Soundset', true);
});
...
mp.keys.bind(Keys.VK_J, false, function () { // belt system
    ...
    if (localplayer.isInAnyVehicle(false)) {
        ...
        if (hudstatus.belt) {
            ...
            mp.events.call('notify', 2, 8, "Вы отстегнули ремень безопасности", 2000);
        }
        else {
...
var fps = mp.game.invoke("0xFC8202EFC642E6F2");
let lastFrameCount = fps;

mp.events.add('UpdatePingStatus', function (ping) {
    fps = mp.game.invoke("0xFC8202EFC642E6F2") - lastFrameCount;
    lastFrameCount = mp.game.invoke("0xFC8202EFC642E6F2");
	mp.gui.execute(`HUD.fps=${fps}`);
    mp.gui.execute(`HUD.ping=${ping}`);
});
...
mp.events.add('render', (nametags) => {
    ...
    if (localplayer.isInAnyVehicle(false)) {
		...
		if(localplayer.vehicle.getPedInSeat(-1) == localplayer.handle) {
			...
			{
				mp.gui.execute(`HUD.inBoat=1`)
			}
			else {
				mp.gui.execute(`HUD.inBoat=0`)
			}
			...
			if (veh.getVariable('PETROL') !== undefined && veh.getVariable('MAXPETROL') !== undefined) {
				...
				if (hudstatus.fuel != petrol && petrol >= 0) {
					...
					global.menu.execute(`petrol.fuel = ${petrol};`);
					...
					if (hudstatus.maxfuel != maxpetrol) {
                    hudstatus.maxfuel = maxpetrol;
					mp.gui.execute(`HUD.maxfuel=${maxpetrol}`);
					}
			...
			var vehicleNames = mp.game.ui.getLabelText(mp.game.vehicle.getDisplayNameFromVehicleModel(veh.model));
			// mp.gui.execute(`HUD.vehiclename=${vehicleName}`);
			
			// var vehicleNames = mp.game.vehicle.getDisplayNameFromVehicleModel(veh.model);
			mp.gui.execute(`HUD.vehiclename=${JSON.stringify(vehicleNames)}`);
