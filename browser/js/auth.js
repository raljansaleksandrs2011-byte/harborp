var slots = new Vue({
    ...
    data: {
        ...
        blockL: ["На улице rain, на душе pain1", "Velikiy_Odmen", "in da future", "in da more future"],
        blockM: ["На улице rain, на душе pain2", "Velikiy_Odmen", "in da future", "in da more future"],
        blockR: ["На улице rain, на душе pain3", "Velikiy_Odmen", "in da future", "in da more future"],
        redbucks: -1,
        login: "username",
    },
});
...
function delchar(data) {
    switch (data) {
        case 1:
            $('.col-l').removeClass('active');
            $('.col-l').removeClass('delete');
            $('.col-l').addClass('free');
			return;
		/*	if ($('#delete-person-1__name-id').val().length <= 2) {
            return console.log('Bad name');
        }

        if ($('#delete-person-1__sername-id').val().length <= 2) {
            return console.log('Bad surname');
        }

        if ($('#delete-person-1__name-id').val().length > 25) {
            return console.log('Bad name');
        }

        if ($('#delete-person-1__sername-id').val().length > 25) {
            return console.log('Bad surname');
        }

        if ($('#delete-person-1__pw-id').val().length === 0) {
            return console.log('Bad pass');
        }*/
        case 2:
            $('.col-m').removeClass('active');
            $('.col-m').removeClass('delete');
            $('.col-m').addClass('free');
            return;
        case 3:
            $('.col-r').removeClass('active');
            $('.col-r').removeClass('delete');
            $('.col-r').addClass('free');
            return;
    }
}
...
function checkAuth(str) {
    ...
    if(str){
        var form_data = str;
        if(typeof form_data != undefined && typeof form_data['remember-me'] != undefined && form_data['remember-me']==true)
        {
            $('#entry-password-id').val(form_data['entry-password']);
            $('#entry-login-id').val(form_data['entry-login']);
            $('#remember-me-id').prop('checked', form_data['remember-me']);
        }
   }
...
$(document).ready(() => {
    ...
    authBtn.on('click', () => {
		...
		//let obj = {
		//		name: $('#entry-login-id').val(form_data['entry-login'])
		//	}
			
			localStorage.setItem('respawnData', authData);
    ...
    $('.js-delete-person-submit-1').on('click', () => {
        ...
        $('.delete-person-page').addClass('show');
        return false;
    });
    ...
    $('.js-delete-cancel').on('click', (e) => {
        e.preventDefault();
        $('.delete-person-page').removeClass('show');
    });
    ...
    $('.js-delete-confirm').on('click', (e) => {
        ...
        $('.delete-person-page').removeClass('show');
			...
			console.error(delCharInfo.slot);
			// Triggered function
			/*switch(delCharInfo.slot) {
				case 1:
                    $('.col-l').removeClass('active');
                    $('.col-l').removeClass('delete');
                    $('.col-l').addClass('free');
                    console.error('delete 1');
                    break;
    ...
    // Update 18.01.21
    $('.button-go').on('click', (e) => {
        e.preventDefault();
        $('.authentication-wrapper__item-animation').removeClass('active');
        $('.authentication-wrapper__item-gangsta__boyzz').removeClass('active');
        $('.authentication').removeClass(e.currentTarget.getAttribute('data-background-remove'));
        $(`#${e.currentTarget.getAttribute('data-id')}`).addClass('active');
        $(`#authentication-login__gangsta-${e.currentTarget.getAttribute('data-gangsta')}`).addClass('active');
        $('.authentication').addClass(e.currentTarget.getAttribute('data-background-add'));
    });

    $('.add-account').on('click', (e) => {
        $('.free').removeClass('create');
        $(`.select-${e.currentTarget.getAttribute('data-id')}`).addClass('create');
    });

    $('.close-create').on('click', (e) => {
        $(`.select-${e.currentTarget.getAttribute('data-id')}`).removeClass('create');
    });

    $('.close-delete').on('click', (e) => {
        $(`.delete-${e.currentTarget.getAttribute('data-id')}`).removeClass('delete');
    });
