...
mp.events.add("vehicleattach", (player, vehicle) => {
	try
	{
		...
		player.attachTo(vehicle.handle, -1, 0, -(c.max.y - c.min.y) / 2 + .4, 1.05, 0, 0, 0, !1, !1, !1, !1, 20, !0);
		//player.clearTasks(), 
...
mp.events.add("entityStreamIn", function(a) {
		...
		setTimeout(() => {
			if(mp.players.exists(a) && 0 !== a.handle && b == a.getVariable("attachToVehicleTrunk")) {
				...
				if(c && 0 !== c.handle) {
					...
					a.attachTo(c.handle, -1, 0, -(b.max.y - b.min.y) / 2 + .4, 1.2, 0, 0, 0, !1, !1, !1, !1, 20, !0)
	...
	try 
	{
		if("vehicle" === a.type && a.hasVariable("freeze")) {
			setTimeout(() => {
				a.freezePosition(a.getVariable("freeze"));
			}, 400)
		}
	}
	catch (e) {}
...
mp.keys.bind(Keys.VK_K, true, function() {
	if (localplayer.vehicle != undefined && localplayer.vehicle.getPedInSeat(-1) == localplayer.handle)
	{
		if (!activedrive)
		{
			...
			if (model == mp.game.joaat('Sultan') || model == mp.game.joaat('sultan') || model == mp.game.joaat('teslapd') || model == mp.game.joaat('teslax') || model == mp.game.joaat('xxxxx'))
			{
				if (x == -1 || y == -1) return;
				activedrive = true;
