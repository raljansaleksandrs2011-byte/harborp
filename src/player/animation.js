...
//     if (alt) {
//         if(animationSlots) mp.events.callRemote('playAnim', animationSlots["1"][0].ad,animationSlots["1"][0].an,animationSlots["1"][0].af); 
//     }
// });
...
setInterval(() => {     
    for(let i = 49; i<=54;i++)
    {
        ...
        if (x&&y) {
            if (animationSlots[i-48].length != 0) { mp.events.callRemote('playAnim', animationSlots[i-48][0].ad, animationSlots[i-48][0].an, animationSlots[i-48][0].af); mp.events.call('clinet::helpkeysonHUD', true, "BACKSPACE", "ОСТАНОВИТЬ АНИМАЦИЮ")};
        }
    }
...
mp.keys.bind(Keys.VK_U, false, function () { // Animations selector   
    ...
    if(OpenAnimation){
        ...
        animation = mp.browsers.new('package://browser/modules/animations/index.html');
...
mp.events.add('playAnim',(animation)=>{
	...
	mp.events.call('clinet::helpkeysonHUD', true, "BACKSPACE", "ОСТАНОВИТЬ АНИМАЦИЮ")
    mp.events.callRemote('playAnim', animation.ad,animation.an,animation.af);

})
mp.events.add('setWalk',(animation)=>{
    animation = JSON.parse(animation)
    // player.setMovementClipset(animation, 0.0);
})
